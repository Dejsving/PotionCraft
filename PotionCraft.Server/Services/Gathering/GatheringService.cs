...
public class GatheringService : IGatheringService
{
        ...
        while (true)
        {
            ...
            if ((roll >= 2 && roll <= 4) || (roll >= 10 && roll <= 12))
            {
                if (_diceRoller.Roll(DiceRoll.D100) > 75)
                {
                    ...
                    return new GatheringResult { Herb = elementalWater, Quantity = 1 };
                }
            }
    ...
    private int DetermineQuantity(AdditionalRulesEnum rule, GatheringRequest request)
    {
        return rule switch
        {
            ...
            AdditionalRulesEnum.NightTwoDayReroll => 2,
            AdditionalRulesEnum.OneOrTwo => _diceRoller.Roll(DiceRoll.D4) % 2 == 0 ? 2 : 1,
            AdditionalRulesEnum.AddElementalWater => 1,
            _ => _diceRoller.Roll(DiceRoll.D4)
        };
    }
}
